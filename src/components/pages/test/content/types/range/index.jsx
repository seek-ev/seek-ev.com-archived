...
const Range = ({ test, editing, submit, setSubmit, patch, loading }) => {
    ...
    const setProperty = (e) => {
        ...
        return setRange({ ...range, [e.name]: e.type === 'number' ? parseFloat(e.value) : e.value })
    }

    const setChargingProperty = (e) => {
        validate(e.name, e.type, e.value, parseFloat(e.min), parseFloat(e.max))
        setCharging({ ...charging, [e.name]: parseFloat(e.value) })
        return setRange({ ...range, chargingTo75Percent: { ...charging, [e.name]: parseFloat(e.value) } })
    }
    ...
    return (
        <Wrapper>
            ...
            <TInput title="Temperature" name="temperature" placeholder="Temperature" value={range.temperature || ''} onChange={setProperty} type="number" step="0.1" min="-50" max="70" error={errors.temperature} disabled={loading || !editing} />
            ...
            <TInput title="Average speed" name="averageSpeed" placeholder="Average speed" value={range.averageSpeed || ''} onChange={setProperty} type="number" step="0.1" min="1" max="300" error={errors.averageSpeed} disabled={loading || !editing} />
            <TInput title="wh per km" name="whPerKm" placeholder="Wh per km" value={range.whPerKm || ''} onChange={setProperty} type="number" step="0.01" min="0" max="1000" error={errors.whPerKm} disabled={loading || !editing} />
            <TInput title="Battery capacity" name="batteryCapacity" placeholder="Battery capacity" value={range.batteryCapacity || ''} onChange={setProperty} type="number" step="0.01" min="0" max="1000" error={errors.batteryCapacity} disabled={loading || !editing} />
            <TInput title="Kilometers" name="km" placeholder="Kilometers" value={range.km || ''} onChange={setProperty} type="number" step="0.1" min="0" max="1000" error={errors.km} disabled={loading || !editing} />
            <TInput title="Miles" name="mi" placeholder="Miles" value={range.mi || ''} onChange={setProperty} type="number" step="0.1" min="0" max="1000" error={errors.mi} disabled={loading || !editing} />
            <TInput title="Range with 75%" name="rangeWith75Percent" placeholder="Range with 75%" value={range.rangeWith75Percent || ''} onChange={setProperty} type="number" step="0.1" min="0" max="1000" error={errors.rangeWith75Percent} disabled={loading || !editing} />
            <TInput title="Charging time to 75%" name="time" placeholder="Charging time to 75%" value={charging.time || ''} onChange={setChargingProperty} type="number" step="0.1" min="0" max="200" error={errors.time} disabled={loading || !editing} />
            <TInput title="Charging speed in kmph" name="kmPerHour" placeholder="Charging speed" value={charging.kmPerHour || ''} onChange={setChargingProperty} type="number" step="0.1" min="0" max="2000" error={errors.kmPerHour} disabled={loading || !editing} />
        </Wrapper>
    )
}
